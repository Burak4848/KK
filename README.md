...
- **Groq (Llama 3) AI Bağlantısı (API):** Projeme ekstra bir özellik olarak "Yapay Zeka" bağlamak istedim. Ancak API kodlarını yazmak karmaşık olduğu için `groq` kütüphanesini kullanarak yemeği analiz ettirme (Prompt atma ve JSON formatında cevap alma) kodlarını yine Yapay Zekadan yardım alarak yazdım.

## Projede Kullandığım Kütüphaneler
- `Flask`: Projeyi bir web sunucusu (site) olarak çalıştırmak için.
- `groq`: Groq (Llama 3) yapay zekasını sisteme bağlamak için.
...
*(Hocama Özel Not: Yapay zeka altyapısı olarak Groq kullandım, bu yüzden çok hızlı ve sorunsuz çalışmaktadır. İstediğiniz kadar test edebilirsiniz!)*
